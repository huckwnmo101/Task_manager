...
const menuItems = [
  { icon: LayoutDashboard, label: "오늘의 할 일", path: "/" },
  { icon: ListTodo, label: "모든 태스크", path: "/tasks" },
  { icon: Folder, label: "프로젝트", path: "/projects" },
  { icon: Tag, label: "카테고리", path: "/categories" },
  { icon: FolderKanban, label: "칸반 보드", path: "/kanban" },
  ...
  if (!user) {
    return (
      <div className="flex items-center justify-center min-h-screen bg-background">
        <div className="flex flex-col items-center gap-8 p-8 max-w-md w-full nordic-fade-in">
          ...
          <Button
            ...
            size="lg"
            className="w-full max-w-xs h-12 rounded-xl font-medium"
          >
            로그인하기
          </Button>
        </div>
  ...
  return (
    <>
      <div className="relative" ref={sidebarRef}>
                ...
                return (
                  <SidebarMenuItem key={item.path}>
                    <SidebarMenuButton
                      ...
                      tooltip={item.label}
                      className={`
                        h-10 rounded-lg transition-all duration-200 font-normal
                        ${isActive
        ...
        <div
          className={`absolute top-0 right-0 w-1 h-full cursor-col-resize hover:bg-primary/20 active:bg-primary/30 transition-colors ${isCollapsed ? "hidden" : ""}`}
          onMouseDown={() => {
      ...
      <SidebarInset className="bg-background">
        ...
        <main className="flex-1 p-6 lg:p-8 nordic-fade-in">
          {children}
        </main>
      </SidebarInset>
    </>
